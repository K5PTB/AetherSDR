...
void CopyAssistController::applyTuning()
{
    ...
    const bool overlapOn =
        CopyAssistSettings::value(QStringLiteral("AsrOverlapEnabled"), QStringLiteral("False")).toString()
        == QStringLiteral("True");
    m_asr->setOverlapMs(overlapOn
        ? CopyAssistSettings::value(QStringLiteral("AsrOverlapMs"), QStringLiteral("500")).toString().toInt()
        : 0);
}
